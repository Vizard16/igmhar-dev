...
import { X, ExternalLink, Github, Youtube, FileText, ChevronLeft, ChevronRight,Briefcase } from "lucide-react"
...
export default function PortfolioSection() {
  ...
  const handleNextImage = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (selectedProject && selectedProject.details && selectedProject.details.images) {
      setCurrentImageIndex((prev) => (prev + 1) % (selectedProject.details?.images.length ?? 1))
    }
  }
  ...
  const handlePrevImage = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (selectedProject && selectedProject.details && selectedProject.details.images) {
      setCurrentImageIndex((prev) => {
        const imagesLength = selectedProject.details?.images?.length ?? 1
      ...
      <div className="container">
        ...
        <div className="flex justify-center mb-12">
          <div className="flex flex-wrap gap-4 justify-center">
            {["All", "Industrial Automation", "Robotics", "Personal Projects"].map((filter) => (
              <button
                key={filter}
                    ...
                    <div className="space-y-3">
                      {selectedProject.urls.map((url, i) => (
                        ...
                        >
                          ...
                          {url.icon.includes("briefcase") && <Briefcase size={18} />}
                          {url.name}
